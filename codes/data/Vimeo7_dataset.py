...
class Vimeo7Dataset(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        self.medium_index_list = []
        for i in range(self.LR_N_frames - 1):
            self.medium_index_list.append(i*2 + 1)
        ...
        # self.paths_GT = pickle.load(open('/media/DATA2/wzr/vimeo_septuplet/vimeo7_train_GT.lmdb/{}'.format(cache_keys), 'rb'))
    ...
    def _init_lmdb(self):
        ...
        #### TODO: change your file path herec
        self.GT_env = lmdb.open('/media/DATA2/wzr/vimeo_septuplet/vimeo7_train_GT.lmdb', readonly=True, lock=False, readahead=False, meminit=False)
        self.LQ_env = lmdb.open('/media/DATA2/wzr/vimeo_septuplet/vimeo7_train_LR7.lmdb', readonly=True, lock=False, readahead=False,
                                meminit=False)
    ...
    def __getitem__(self, index):
        ...
        self.medium_frames_list = []
        for i in self.medium_index_list:
            self.medium_frames_list.append(neighbor_list[i])
       ...
       #### get LQ images
        ...
        LQ_size_tuple = (3, 64, 112) if self.LR_input else (3, 256, 448)
        img_medium_l = []
        for v in self.medium_frames_list:
            if self.data_type == 'mc':
                img_medium = self._read_img_mc(
                    osp.join(self.LQ_root, name_a, name_b, '/{}.png'.format(v)))
                img_medium = img_medium.astype(np.float32) / 255.
            elif self.data_type == 'lmdb':
                img_medium = util.read_img(self.LQ_env, key + '_{}'.format(v), LQ_size_tuple)
            else:
                img_medium = util.read_img(None,
                                       osp.join(self.LQ_root, name_a, name_b, 'im{}.png'.format(v)))
            img_medium_l.append(img_medium)
        ...
        if self.opt['phase'] == 'train':
            ...
            if self.LR_input:
                ...
                img_medium_l = [v[rnd_h:rnd_h + LQ_size, rnd_w:rnd_w + LQ_size, :] for v in img_medium_l]
                ...
                img_medium_l = [v[rnd_h:rnd_h + GT_size, rnd_w:rnd_w + GT_size, :] for v in img_medium_l]
                img_GT_l = [v[rnd_h:rnd_h + GT_size, rnd_w:rnd_w + GT_size, :] for v in img_GT_l]

            # augmentation - flip, rotate
            img_LQ_l = img_LQ_l + img_medium_l + img_GT_l
            rlt = util.augment(img_LQ_l, self.opt['use_flip'], self.opt['use_rot'])
            img_LQ_l = rlt[0:self.LR_N_frames]
            img_medium_l = rlt[self.LR_N_frames:-N_frames]
            img_GT_l = rlt[-N_frames:]

        # stack LQ images to NHWC, N is the frame number
        img_LQs = np.stack(img_LQ_l, axis=0)
        img_GTs = np.stack(img_GT_l, axis=0)
        img_mediums = np.stack(img_medium_l, axis=0)
        # BGR to RGB, HWC to CHW, numpy to tensor
        img_GTs = img_GTs[:, :, :, [2, 1, 0]]
        img_LQs = img_LQs[:, :, :, [2, 1, 0]]
        img_mediums = img_mediums[:, :, :, [2, 1, 0]]
        ...
        img_LQs = torch.from_numpy(np.ascontiguousarray(np.transpose(img_LQs,
                                                                     (0, 3, 1, 2)))).float()
        img_mediums = torch.from_numpy(np.ascontiguousarray(np.transpose(img_mediums,
                                                                     (0, 3, 1, 2)))).float()

        return {'LQs': img_LQs, 'mediums': img_mediums, 'GT': img_GTs, 'key': key}
