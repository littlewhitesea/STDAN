...
__device__ scalar_t dmcn_im2col_bilinear2(
...
template <typename scalar_t>
__device__ scalar_t dmcn_get_gradient_weight2(
    scalar_t argmax_h,
    scalar_t argmax_w,
    const int h,
    const int w,
...
template <typename scalar_t>
__device__ scalar_t dmcn_get_coordinate_weight2(
    scalar_t argmax_h,
    scalar_t argmax_w,
    const int height,
    const int width,
...
template <typename scalar_t>
__global__ void modulated_deformable_im2col_gpu_kernel2(
    const int n,
...
template <typename scalar_t>
__global__ void modulated_deformable_col2im_gpu_kernel2(
    const int n,
    ...
    for (int dy = -2; dy <= 2; dy++) {
      for (int dx = -2; dx <= 2; dx++) {
        if (cur_h + dy >= 0 && cur_h + dy < height && cur_w + dx >= 0 &&
          ...
          scalar_t weight = dmcn_get_gradient_weight2(
              cur_inv_h_data,
              cur_inv_w_data,
              cur_h + dy,
              cur_w + dx,
...
template <typename scalar_t>
__global__ void modulated_deformable_col2im_coord_gpu_kernel2(
    const int n,
    const scalar_t* data_col,
    const scalar_t* data_im,
    const scalar_t* data_offset,
  ...
  CUDA_KERNEL_LOOP(index, n) {
    ...
    for (int col_c = (offset_c / 2); col_c < channel_per_deformable_group;
      ...
      const scalar_t weight = dmcn_get_coordinate_weight2(
          inv_h,
          inv_w,
          height,
          width,
...
void modulated_deformable_im2col_cuda2(
  ...
  if (err != cudaSuccess) {
    printf(
        "error in modulated_deformable_im2col_cuda2: %s\n",
        cudaGetErrorString(err));
  }
}
...
void modulated_deformable_col2im_cuda2(
  ...
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(
      data_col.scalar_type(), "modulated_deformable_col2im_gpu", ([&] {
        ...
        modulated_deformable_col2im_gpu_kernel2<<<
            GET_BLOCKS(num_kernels),
            CUDA_NUM_THREADS,
            0,
            stream>>>(
  ...
  if (err != cudaSuccess) {
    printf(
        "error in modulated_deformable_col2im_cuda2: %s\n",
        cudaGetErrorString(err));
  }
}
...
void modulated_deformable_col2im_coord_cuda2(
  ...
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(
      data_col.scalar_type(), "modulated_deformable_col2im_coord_gpu", ([&] {
        ...
        modulated_deformable_col2im_coord_gpu_kernel2<<<
            GET_BLOCKS(num_kernels),
            CUDA_NUM_THREADS,
            0,
            stream>>>(
  ...
  if (err != cudaSuccess) {
    printf(
        "error in modulated_deformable_col2im_coord_cuda2: %s\n",
        cudaGetErrorString(err));
  }
}