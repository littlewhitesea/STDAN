...
def main():
    ...
    with open(test_dataset_folder + '/slow_testset.txt', 'r') as f:  #'slow_testset.txt' #'medium_test.txt' #'fast_test.txt'
        sub_folder_l_list = f.read().splitlines()

    sub_folder_l = []
    folder_list_num = len(sub_folder_l_list)
    for ii in range(folder_list_num):
        temp = os.path.join(test_dataset_folder_LR, sub_folder_l_list[ii])
    ...
    logger.info('Data: {} - {}'.format(data_mode, test_dataset_folder_LR))
    logger.info('Padding mode: {}'.format(padding))
    logger.info('Model path: {}'.format(model_path))
    logger.info('Save images: {}'.format(save_imgs))
    logger.info('Flip Test: {}'.format(flip_test))
